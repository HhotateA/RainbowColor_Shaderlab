Shader "HOTATE/Wave2RGB" {
    ...
    SubShader {
        ...
        CGINCLUDE
            ...
            //data on http://cvrl.ucl.ac.uk/cmfs.htm //fixed4(wavelength,R,G,B)元のデータの構造由来
            #define minwave 390.0
            #define  datanum 89
            #define interval 5.0
            static fixed4 CMF[89]={
            ...
            fixed4 Wave2RGB(float wavelength){
                wavelength = clamp( wavelength, minwave, minwave+interval*datanum);
                int index = clamp( (int)(wavelength-minwave)/interval, 0, datanum);
                float waight = saturate((wavelength - minwave - index * interval)/interval);
                fixed4 output = lerp(CMF[index],CMF[index+1],waight);
                return fixed4(output.yzw,1.0);
            }

            fixed4 Val2RGB(float val){
